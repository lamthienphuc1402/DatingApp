...
const MatchedUsersList = ({ onSelectUser }) => {
  ...
  return (
    <div className="matched-users-list bg-white rounded-lg shadow-md p-4">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">Danh sách đã match</h2>
      <div className="mb-4">
        <input
          type="text"
      ...
      {matchedUsers.length === 0 ? (
        <p className="text-gray-500 text-center">Chưa có người dùng nào được match.</p>
      ) : (
        <motion.div
          ...
          {currentUsers.map((user) => (
            ...
            >
              <div className="relative">
                ...
                />
                <span className={`absolute bottom-0 right-0 w-3 h-3 rounded-full ${user.isOnline ? 'bg-green-500' : 'bg-gray-400'}`}></span>
              </div>
              <div className="ml-3 flex-grow">
                <span className="font-semibold text-gray-800">{user.name}</span>
                <p className="text-sm text-gray-500 truncate">{user.bio || 'Không có tiểu sử'}</p>
              </div>
        ...
        {Array.from({ length: Math.ceil(filteredUsers.length / usersPerPage) }).map((_, index) => (
          <button
            ...
            className={`mx-1 px-3 py-1 rounded ${
              currentPage === index + 1 ? 'bg-pink-500 text-white' : 'bg-gray-200 text-gray-700'
            }`}
          >
            {index + 1}
          </button>
