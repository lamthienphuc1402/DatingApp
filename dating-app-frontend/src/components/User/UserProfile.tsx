            ...
            <>
              <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-8">
                ...
                <div className="flex-1 text-center md:text-left space-y-3">
                  <h2 className="text-4xl md:text-5xl font-bold text-gray-800 mb-2">{user.name}</h2>
                  <p className="text-lg text-gray-600">{user.email}</p>
                  <p className="text-lg text-gray-700 italic">"{user.bio || 'Chưa có thông tin tiểu sử.'}"</p>
                </div>
              </div>
                ...
                <div className="bg-purple-50 rounded-2xl p-6 transform transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-3 md:gap-4">
                    <div>
                      ...
                      <p className="text-lg font-medium text-purple-800">{user.age || 'Chưa cập nhật'}</p>
                    </div>
                    <div>
                      <p className="text-purple-600">Cung hoàng đạo</p>
                      <p className="text-lg font-medium text-purple-800">{user.zodiacSign || 'Chưa cập nhật'}</p>
                    </div>
                    <div>
                      <p className="text-purple-600">Học vấn</p>
                      <p className="text-lg font-medium text-purple-800">{user.education || 'Chưa cập nhật'}</p>
                    </div>
                    <div>
                ...
                <div className="bg-pink-50 rounded-2xl p-6 transform transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {user.interests && user.interests.length > 0 ? (
                      ...
                      <p className="text-base md:text-lg text-pink-700">Chưa có thông tin về sở thích.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-red-50 rounded-2xl p-6 transform transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                    {user.profilePictures && user.profilePictures.length > 0 ? (
                      ...
                      <p className="text-lg text-red-700 col-span-3">Chưa có hình ảnh.</p>
                    )}
                  </div>
                </div>
